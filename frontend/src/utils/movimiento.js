// Física de interfaz al estilo Apple (WWDC "Designing Fluid Interfaces"), la misma
// de NutriFit. Resortes interrumpibles: siempre parten del valor y la velocidad
// actuales, así un gesto puede agarrar un elemento a mitad de animación sin saltos.
...
// Háptica ligera donde el navegador la soporte (Android). Solo para momentos que lo merecen.
export const haptic = (ms = 10) => {
  if (navigator.userActivation && !navigator.userActivation.hasBeenActive) return
  try { navigator.vibrate?.(ms) } catch { /* no soportado */ }
}

// Sigue un gesto desde pointerdown hasta que se suelta, aunque el puntero salga
// del elemento. Si hubo arrastre, se traga el click que el navegador dispara al soltar.
